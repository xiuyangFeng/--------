...
# 定义Pinn_Cfd_Blood类，用于血流仿真中的物理信息神经网络（PINN）
class Pinn_Cfd_Blood:
    ...
    ## compute_fun_loss and data pre-processing
    # 定义计算函数损失和数据预处理函数
    ...
    ## shuffle data and get self.point_number of data
    # 定义数据打乱和重置epoch损失的函数
    ...
    def train(self, mdoel_data_sets):
        ...
        for counter, epoch_value in enumerate(self.epochs):
            ...
            for e in range(1, epoch_value + 1):
                ...
                if self.loss_history['total'][number_epochs] < self.loss_epochs:
                    torch.save(self.net.state_dict(),'N/' + 'PINN_CFD_BLOOD_weight_sss.pth')
                    self.loss_epochs = self.loss_history['total'][number_epochs]
                self.net_params = 'N/' + 'PINN_CFD_BLOOD_weight_sss.pth'
        ...
        loss_history.to_csv('N/history_sss.csv', index=False)
...
def main():
    ...
    # 设置训练的超参数
    res = [0.1, 0.1]  # p,wss
    loss_weights_Data =  [40.0, 25.0]
    loss_weights_Equ = [1.0, 1.0, 40.0, 1.0]
    data_former = [1, 1, 1, 1]
    epochs = [500, 500, 500]
    batch_sizes = [15000]
    ...
    [深度神经网络]
    使用mape时，始终降不到1以下，而且对于压力场和用mse优化时差距较大；
    对单个物理场进行优化时，速度场里需要学习率较低，因为数值小的较多，发现对单独网络优化时也会受到其他物理场影响
    
    进行网咯结构的调整
    如果将uvw设一个网络，p设一个网络，则误差会先优化uvw，此时uvwp效果较好
    uvwp设为一个网络，则误差会先优化p
    uv共享一个底层权重，wp共享一个底层权重，此时效果较好，但没有第1种效果好
    若uvwp分别设置一个pointnet网络，但是在底层共享一个权重，则效果和设置一个pointnet网络效果类似
    分别设置一个pointent网络，但是不共享权重，使用mae优化uvw，mse优化p
