...
区宽 = max(11 样本, rint(min(R_end, 段中位 R, 12 mm) / 0.5 mm))，下限取一个完整 SG 窗，保证 1～3 个样本的钩回连同其 SG 足迹
全部落在区内（首版下限 6 样本时，SUN_XU_XIA-1 与 HOU_SHI_GUO-0 的区外第一个样本仍残留 0.12～0.15 的尾巴）；
区内切线取内侧 1R～2R 带的切线均值，曲率取该带中位数。
...
| 子支起点（分叉） | 1,036 | 5.5 / 7.5 / 12.0 | 0.073 / 0.173 / 0.470 |
| 出口端 | 692 | 5.5 / 5.5 / 8.0 | 0.046 / 0.112 / 0.507 |

逐例区内样本占比中位 11.6%，最大 17.3%；改前区内最大曲率超过 0.3 /mm 的端占 1.7%；2 个段侧因过短跳过。

**重建后全队列**：曲率 p50/p99/max 0.0356 / 0.1545 / 0.4448（此前 0.0358 / 0.1677 / 0.5517），train138 p99/max 0.1548 / 0.4448；
`k·R` p99/max 0.98 / 5.25；kink Gate interior/junction/endpoint 全为 0；review 队列空；峰值位置 endpoint 0 / junction 36 / interior 137。
原先四例：ZHANG_XUN_LIAN 0.417→0.288、LIU_FENG 0.552→0.259、SUN_XU_XIA-1 0.402→0.180、HOU_SHI_GUO-0 0.401→0.222。
