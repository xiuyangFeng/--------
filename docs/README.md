...
- **WSS-only 最小化线**：`pipeline_wss_min/` + `training_wss_min/`；第三轮 clean-data 已完训，当前重点从数据纠错转为选模稳定性、尺度/边界条件信息上限和 high-WSS 峰值恢复
...
| WSS-only 最小化预处理/训练 | [WSS 最小化训练实验跟踪](02-推进与变更/WSS最小化_训练实验跟踪.md) + [WSS 最小化推进记录](02-推进与变更/WSS最小化_代码修改与实验推进记录.md) + [`pipeline_wss_min` README](../pipeline_wss_min/README.md) + [`training_wss_min` README](../training_wss_min/README.md) | 第三轮 clean-data 已完训：target-weight 三 seed test `R²_field=0.225±0.034`，high-WSS 仍 No-Go；下一步先修选模并做尺度/边界条件信息探针 |
...
| WSS-only 最小化 | [训练实验跟踪](02-推进与变更/WSS最小化_训练实验跟踪.md) · [代码修改与实验推进记录](02-推进与变更/WSS最小化_代码修改与实验推进记录.md) · [历史诊断与交接归档](02-推进与变更/_archive/WSS最小化/README.md) |
