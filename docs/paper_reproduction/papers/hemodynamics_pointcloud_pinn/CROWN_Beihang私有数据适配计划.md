...
> 当前状态：raw_ascii v1 预处理 **5738 完成** · **5739 No-Go（OOM 截断）** · **5740/5751 evaluate 后 No-Go** · lazy 加载 **已审核** · 旧 5625/5626 **作废**（见推进记录）。
...
CROWN/Beihang 源码第一轮只做“脑血管小样本 PointNet-PINN 速度/压力复现线”：完整预测 `u,v,w,p`，比较 PINN 与非 PINN，判断该论文路线在本项目私有数据上的速度场/压力场可学性。**2026-06-21 结论**：5751 非 PINN 充分训练（best_ep=55）后 `p_r2=-5.28`、`nmae=0.077`，压力仍 No-Go；5740 PINN 亦 No-Go。CROWN 仅作外部速度/压力复现对照，**不进入第二阶段 WSS**。5739 保留 OOM 截断记录。适配时最重要的是把原论文源码复现、私有数据中间层、显式几何特征保留、后续 WSS 扩展分开。
