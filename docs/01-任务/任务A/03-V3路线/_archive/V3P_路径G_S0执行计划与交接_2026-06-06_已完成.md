# V3P 路径 G · S0 执行计划与交接（G0 oracle + 标签 QA · 0 重训）· 已完成

> **归档日期**：2026-06-12 · **最终结论**：S0 ✅ 全闭合；S1 全部分支已跑完并封口
...
| 阶段 | 结果 |
| --- | --- |
| S0 待办 1–6（Job **5409**） | ✅ 全完成 |
| QA + denylist B | ✅ 标签池闭合（train 57 / test 16） |
| G1-a0 VNHeadPlain（**5441**） | ❌ **No-Go 封口** |
| G2-a1 固定 WSS₀ | ❌ **关闭**（G0-d No-Go） |
| G2-b kernel-attention（**5410/5437/5438**） | ❌ **No-Go 封口**（0.404±0.012 < 基线 0.434） |
| G3 SSL 预训练（**5474/5475**） | ❌ **No-Go 归档** |
| G5 eval（**5459/5460**） | ✅ Pa p95 **0.559** |
| post5463 基线 band | **0.425±0.012**（5466/5468/5478） |

**下一跳**：主线转 **G4 2D 换轨预研**（Phase 0 ✅ · Phase 1 进行中）。
...
### 9.2 路径分叉（按路径 G §9/§11）— 最终封口（2026-06-12）

| 分支 | 决策 | 下一动作 | GPU |
| --- | --- | --- | --- |
| **G1 等变头** | **No-Go · 封口归档**（5441 · 矢量分量崩，证实 G0-a） | 不再投 | ✅ |
| **G2-a 残差** | **关闭**（G0-d） | 不排固定 WSS₀ 头 | — |
| **G2-b 压力耦合** | **No-Go · 封口**（5437/5438/5410 · 0.404±0.012） | 不再投 | ✅ |
| **G3 预训练/多保真** | **No-Go · 归档**（5474/5475） | 已归档 | ✅ |
| **G4 2D 换轨** | **主线**（G0-b `pod_2d` 0.672） | Phase 0 ✅ · Phase 1 进行中 | 预研 |
| **G5 兜底叙事** | **eval 闭环**（Pa p95 0.559） | 低优先级维护 | ✅ |

### 9.3 S1 推荐执行顺序 — 已全部执行完毕

1. ~~G2-b head 实现 + Probe~~ → **No-Go 封口**
2. ~~`extract_features` STL 尺度审计~~ → ✅
3. ~~G3 几何 SSL 预研~~ → **No-Go 归档**
4. ~~5311 clinical-pa eval~~ → ✅ G5 闭环
5. G1-a0 / G2-a1 不重开 → ✅ 已封口
...
按 `experiment-log.mdc`，完成实跑后在 [代码修改与实验推进记录.md](../../../../02-推进与变更/代码修改与实验推进记录.md) **文首**追加，并同步 [实验跟踪日志](../01-执行与待办/V3_实验执行跟踪日志.md) 顶部、[后续优化待办](../01-执行与待办/V3_后续优化待办.md) 对应行（新增 TODO-54/56 行状态）、[README](../README.md) §0 当前状态。
