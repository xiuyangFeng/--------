...
> 当前状态：**单目标 Probe 完成 → CFD 异常病例 `fast/PENG_JI_MING` 已定位并移除 → 排除后 P R²=0.96, WSS R²=0.40 → 可进入下一层**（2026-05-05）；
> **文档索引**：[README.md](../README.md)（推荐入口）
> **精度突破规划**：[V3_精度突破路径与发散方案.md](V3_精度突破路径与发散方案.md)（2026-05-23）
> 实验跟踪：[V3_实验执行跟踪日志.md](../01-执行与待办/V3_实验执行跟踪日志.md) · 待办：[V3_后续优化待办.md](../01-执行与待办/V3_后续优化待办.md) · 三域数据：[V3_三域数据.md](../03-数据与质量/V3_三域数据.md)
> 数据质量（V3P OOD）：[V3_数据质量与OOD诊断记录.md](../03-数据与质量/V3_数据质量与OOD诊断记录.md)
>
**快速导航**：§1 定位 · §2 数据锁定 · §6 实验矩阵 · §7 Go/No-Go · §8 实现 · §10 结论 · §11 前置清单
>
> **数据变更（2026-05-05）**：`fast/PENG_JI_MING` 因 CFD 原始数据异常（入口流速 ~0, WSS 275× 超常）已从 `split_AG_v1.json` 移除（训练/测试均不含）。待原始 CFD 数据修复后可重新纳入。图资产需重新生成以确保归一化统计量排除该病例。
