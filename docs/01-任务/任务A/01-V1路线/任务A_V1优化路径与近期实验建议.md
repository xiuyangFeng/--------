...
> 上位文档：[任务A实验清单](任务A_V1实验清单.md) | 相关文档：[任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md) / [任务A冻结卡](任务A_V1冻结卡.md)
...
> **2026-04-24 补充说明**：**`V2P-WSSP-01`**（PointNeXt，**Line W 向壁面专线**：**仅 p + WSS 监督**，壁面-heavy 采样）seed=1 已归档。`summary` 上 **压力拟合正常**（`r2_p≈0.91`），**速度 R²≈0 属预期**（未监督）；**WSS 以 `wall` 区 JSON 为主**（`wall.r2_wss` 约 0.44，bootstrap）。与 **`A-*-wss-multi`**（场全监督 + 小权重 WSS）**不可混表**。详见 [任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md)「V2P-WSSP-01」。

> **2026-04-25 补充**：**`V2P-WSSP-02`**（**全场监督 + WSS 头 + 混合早停**，标准采样，seed=1）已归档。**`r2_vel_mag` 明显优于 WSSP-01**；**`wss_r2_wss` 在 `summary` 上仍略负**，**未**在测试集上超过 WSSP-01 的 **`wall.r2_wss`**（且两者采样与损失不同）。详见 [任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md)「V2P-WSSP-02」。
...
> **2026-04-26 补充**：**`V2P-WSSP-03` / `V2P-WSSP-04`** seed=1（**标准采样**，bootstrap）已闭环。**03** 修复 **WSSP-02** 的 **`r2_p` 崩溃**；**04** 相对 **03** 在 **`r2_p` 与 `wall` WSS** 上**无收益**，**`r2_vel_mag` 略差**。详见 [任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md)「实验记录摘要 · V2P-WSSP-03 / V2P-WSSP-04」与 **`outputs/field/plots/v2p_wssp03_vs_04_seed1/`**。

> **2026-04-27 补充**：**Line W 权重复扫**（**`A-Opt-W03-p025-w02` / `w02` / `w05`**）与 **V2P-WSSP-04** 已同批分区域+多模型对比；**W03 三线**压力与 WSS 变化均有限，**`w02` 在 wall WSS 上略好**；**V2P-04 分区域 `r2_p` 与 Transformer+场全监督线不在同档**，**勿混表**。总览图 **`outputs/field/plots/wssp04_w03_line_seed1_20260427/`**；表格式结论见 [任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md)「**Line W：A-Opt-W03 权重复扫**」、 [代码修改与实验推进记录](../../../02-推进与变更/代码修改与实验推进记录.md) **2026-04-27** 条目。

> **2026-04-28 补充**：**`V2P-WSSP-05` / `V2P-WSSP-06`**（**仅 p+WSS**，**MSE vs Huber** 单变量，各三 seed）已完成训练与主后处理；**`summary.test_metrics` 上 `wss_r2_wss` 仍略负**，**Huber 未见稳定抬升**；**`r2_p` 跨 seed 波动大**。**`V2P-WSSP-06` seed=3** 可先补 **`predict_field`** 再与 05 对齐比较。详见 [任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md)「实验记录摘要 · V2P-WSSP-05 / V2P-WSSP-06」、[代码修改与实验推进记录](../../../02-推进与变更/代码修改与实验推进记录.md) **2026-04-28**。

> **2026-04-22 补充说明**：**V2P Bootstrap 首轮 seed=1 已完成**。`V2P-Base-01`（无 geometry）`r2_vel_mag=0.354`，`V2P-Main-01`（有 geometry）`r2_vel_mag=0.609`；geometry 增益显著（+72%），V2P-Main-01 已在全局 `r2_vel_mag` 口径上超过 V1 锚点 `A-Opt-05`。当前使用 `split_AG_v1`（bootstrap），正式结论等 Gate-0 通过后在 `split_AG_v2` 复现。详见 [任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md)「V2P Bootstrap」与 [代码修改与实验推进记录](../../../02-推进与变更/代码修改与实验推进记录.md) 2026-04-22 条目。

> **2026-04-18 补充说明**：关于“是否从旧 V1 单尺度 GNN 主线切到 PointCloud / MeshGNN”的详细执行路线，已单独写入 [任务A V2修正路线实验矩阵](../02-V2路线/任务A_V2修正路线实验矩阵.md) 第 **5.4 节**。当前建议是：**停止继续扩展旧 `kNN + GNN` 主线，先完成 V2 Gate-0，再以 PointCloud 作为首个可训练主线，MeshGNN 作为 Gate-0 通过后的受控对照线。**
...
两条线共享同一套 **P0 组合训练配方**（`target_weights` + Pre-Norm；**`A-Opt-03w` 未更好**）。**（2026-03-31）默认起跑配置（母版 backbone）统一为 `A-Opt-05`**（`hidden_dim=256`，`num_layers=4`）：**`interior` / `near_wall` 等略优于 `A-Opt-03`**，更贴近 WSS 前置质量；**`A-Opt-03`（h128）** 作 **轻量/效率对照**。详见 [任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md)「战略锚点」。各自独立归因不变。
...
1. 先在当前 Transformer 残差块中补 `LayerNorm`，采用 Pre-Norm 风格。（**已完成**：`A-Opt-02` 三 seed，结论见下文 **P0-2** 与 [任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md)。）
...
- **Run 目录**：`outputs/field/_archive/legacy_pre_wss_schema/field_transformer_coord_t_bc_geom_wall_prenorm_split_AG_v1_seed{1,2,3}_20260327_*`（`A-Opt-02`，`model.use_transformer_prenorm: true`，其余与 `A-Main-01` 对齐）。
...
- **P0-3（已完成，2026-03-27）**：实验 **`A-Opt-02_warmup`** 三 seed 已训练并归档；run 目录 `outputs/field/_archive/legacy_pre_wss_schema/field_transformer_coord_t_bc_geom_wall_prenorm_warmup5_split_AG_v1_seed{1,2,3}_20260327_*`。结论摘要见本节 **「P0-3 实验结论摘要」** 与 [任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md)「实验记录摘要 · A-Opt-02_warmup」。三模型对照图：**`outputs/field/plots/optimization/prenorm_Main_P02_P02w/`**（`python -m training.scripts.regenerate_p02_warmup_comparison_figures` 可重生成）。
...
**与本仓库的对齐（2026-03-27，结论延展 2026-03-31）**：P0-3 已落地为 **`A-Opt-02_warmup`**（在 **`A-Opt-02`** 上仅打开上述 warmup），与 [任务A冻结卡](任务A_V1冻结卡.md)、[任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md) 第三批推荐顺序一致。**`A-Opt-02_warmup`（三 seed）已归档**。**`A-Opt-03` / `A-Opt-03w`（P0-4）已于 2026-03-28 归档**。**`A-Opt-04` / `A-Opt-05`（P0-5）已于 2026-03-29 归档**。**（2026-03-31）新开跑母版切换为 `A-Opt-05`**（见状态表「战略锚点」）。训练复现示例（P0-3）：
...
- **`A-Abl-02` 已完成（2026-04-09）**：**NormRadius** 主贡献、**Tangent** 次之，**Abscissa / Curvature** 边际较弱（详见 [任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md)）；后续新增特征仍与 **`A-Opt-05`** 一对一对照；
...
| --- | --- | --- | --- | --- | --- | --- |
| `A-Opt-G01` | 距分叉点距离 + 分叉拓扑标记 | 显式编码局部拓扑转折，告诉模型“这里离分叉还有多远” | `bifurcation`、`trunk->branch` 过渡区 | 低 | 低 | **✅ 三 seed（2026-04-14）**；相对 05：`summary.rmse_vel_mag` 略差、`R²_u/p` 略好——见 [任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md) |
| `A-Opt-G02` | 半径变化率 / 截面积变化率 | 比 `NormRadius` 更进一步，描述扩张、收缩和局部几何突变 | `bifurcation`、瘤腔扩张区、近壁剪切变化区 | 中 | 中 | **✅ seed1**；**⏹ 已结案**（2026-04-17：相对 `A-Opt-05` 无收益，**不补 seed2/3**） |
| `A-Opt-G03` | 扭率 `torsion` | 补充曲率无法表达的三维空间扭转信息 | 三维弯扭段、高曲率区 | 中 | 中 | **✅ seed1**；**⏹ 已结案**（同上） |
| `A-Opt-G04` | 壁面距离相关特征 | 直接告诉模型节点离壁面有多远，服务近壁速度剖面学习 | `near_wall`、WSS 相关区域 | 中 | 低 | **✅ 三 seed**；`summary.rmse_vel_mag` 略优于 05 |
| `A-Opt-G05` | 中心线方向变化率 | 比单点 `Tangent` 更强调局部流向转折强度 | 高曲率段、入口下游扰动区 | 中 | 中 | **✅ 三 seed**；`summary.rmse_vel_mag` 略优于 05 |

> **不建议优先做的事情**：一口气增加 3~5 类新特征；加入定义口径不稳定的高阶导数几何量；在样本量不大的情况下堆很多彼此强相关的几何列。

**Line G 推荐执行顺序：**（**前置 `A-Abl-02` 已于 2026-04-09 完成**：**NormRadius / Tangent** 主贡献，见 [任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md)）

1. ~~`A-Opt-G01`~~：**✅ 三 seed（2026-04-14）**
2. ~~`A-Opt-G02`~~：**✅ seed1**；**⏹ 已结案**（不补种）
3. ~~`A-Opt-G03`~~：**✅ seed1**；**⏹ 已结案**（不补种）；~~`A-Opt-G04`~~：**✅ 三 seed（2026-04-13）**
4. ~~`A-Opt-G05`~~：**✅ 三 seed（2026-04-13）**
...
3. `A-Opt-07`：区域加权 `interior_boost = 3.0`（**✅ 2026-04-02；负结果**—相对 **`A-Opt-05`** 内部/全图 **`RMSE_|v|`** 未改善，**壁面/近壁变差**；多模型图 **`plots/optimization/A_Opt07_vs_Opt05_Main01/`**）

**（2026-03-29）** 已观测到 **加宽单步伤害内部速度、加深部分回补** 的模式——**应停止在 6L 上继续堆**，转向 **区域加权 / `A-Abl-02` / Line G / 多尺度立项**。**（2026-04-09）`A-Abl-02` 已闭环**，后续转向 **`A-Abl-01` / Line G**。
...
7. **`A-Abl-02`**：**（✅ 2026-04-09）** 已在 **`A-Opt-05` 母版**上完成四子组 × 三 seed；**`A-Opt-07`（2026-04-02）** 结论为负，不影响 02 叙事
8. **下一优先**：`A-Abl-01`（输入层级）；**`A-*-wss-multi` 预测与 Fig A3–A5 ✅（2026-04-17，Job 2704/2715）**；**R² 汇报表**见 [任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md)「实验记录摘要 · WSS 多任务」；或 **Line W**（W01+）
9. ~~`A-Opt-G01`~~ / ~~`G04`~~ / ~~`G05`~~：**三 seed 已归档（2026-04-14，见状态表「Line G」）**
10. ~~`A-Opt-G02/G03`~~：半径变化率 / 扭率——**⏹ 已结案**（仅 seed1，不补种，见状态表第四批）
11. **（2026-04-17）** **`A-*-wss-multi`**（场 + WSS 头，`baseline_wss_multitask/`）训练、**壁面 WSS 表**、**`predictions_test` 全链**已入账；**R²（壁面 WSS / \|v\| / p）主表**见状态表「实验记录摘要 · WSS 多任务」；与 **`line_g`** 独立——见状态表「第五批附」
12. 若单尺度收益趋缓，再立项多尺度结构（A-Opt-10）；**默认暂缓 `A-Opt-06`（6L）**
...
| A-Opt-07 | 内部点区域加权是否改善瓶颈 | `interior_boost=3.0` | 代码 ~30行 | P1（✅ 2026-04-02；**相对 `A-Opt-05` 未改善**） |
...
建议先把 `A-Opt-01 ~ A-Opt-07` 作为近期主线，其余根据结果再决定是否进入正式执行。**（2026-04-02）`A-Opt-07` 已归档**：结论为**负**。**（2026-04-09）`A-Abl-02` 已归档**。**（2026-04-14）Line G：`G01`/`G04`/`G05` 三 seed 已归档**；**（2026-04-17）`G02`/`G03` ⏹ 已结案**（仅 seed1，不补种）。**（2026-04-17）** **`A-*-wss-multi`**（场 + `wss_loss_weight=0.1`，配置 **`baseline_wss_multitask/`**）训练与壁面 WSS 表已入账——见 [任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md)「第五批附」（与 **`line_g`** 独立）。后续主线仍以 **`A-Opt-05`** 为母版，优先 **`A-Abl-01`**、**按需 `plot_taskA_multimodel_regional_bar`（wss-multi 已具备 `regional_eval`）** 或 **Line W**（W01+）。

### 显式几何增强线 Line G（2026-03-26 新增）

| Exp ID | 研究问题 | 唯一变化项 | 改动类型 | 建议优先级 |
| --- | --- | --- | --- | --- |
| A-Opt-G01 | 显式分叉拓扑先验是否改善复杂转折区建模 | `dist_to_bifurcation + branch_id`（与 `pipeline` / JSON 一致） | 预处理 + 配置 | PG0（**✅ 三 seed，2026-04-14**） |
| A-Opt-G02 | 局部尺度变化信息是否优于单纯半径值 | `dR_ds` | 预处理 + 配置 | PG1（**✅ seed1**；**⏹ 已结案**，不补种） |
| A-Opt-G03 | 扭率能否补足曲率缺失的三维弯扭信息 | `torsion` | 预处理 + 配置 | PG1（**✅ seed1**；**⏹ 已结案**，不补种） |
| A-Opt-G04 | 显式壁面距离是否改善近壁速度剖面学习 | `dist_to_wall` 等 | 预处理 + 配置 | PG1（**✅ 三 seed，2026-04-13**） |
| A-Opt-G05 | 中心线方向变化率是否提升转折区表达 | `d_tangent_ds` | 预处理 + 配置 | PG2（**✅ 三 seed，2026-04-13**） |
...
| A-Opt-W03 | 直接 WSS 监督是否最大化端到端质量 | **纯 WSS**：`target_weights=0` + `wss_loss_weight=1`（草案 `A-Opt-W03-wss-only`）；**场+WSS 联合**见已跑 **`A-*-wss-multi`**（`wss_loss_weight=0.1`） | 代码 ~120行 + 数据管线 | PW2 |
| A-Opt-W04 | 两阶段训练是否优于一开始就加权 | 阶段1:均匀MSE → 阶段2:壁面精调 | 代码 ~50行 | PW3 |
| A-Opt-W05 | OSI 敏感区加权是否改善 OSI 恢复 | 分叉/高曲率区 boost | 代码 ~20行 | PW4 |

建议 `A-Opt-W01 ~ W02` 作为 Line W 近期主线。**（2026-04-17）** **`A-*-wss-multi`** 已提供「场 + WSS 头」先行证据（`outputs/field/wss_multitask_test_wall_wss_metrics.tsv`）；**纯 WSS**（`A-Opt-W03-wss-only`）仍视需求单开。`W03` 表格行勿与 **`wss-multi` 联合监督**混名。
