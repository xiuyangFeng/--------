...
> **2026-04-20 执行顺序补充**：虽然首轮主表仍保留 `V2G-*` 与 `V2P-*` 两条路线，但当前实际推进顺序已调整为：**先完成 PointCloud 首轮最小闭环，再决定是否进入 MeshGNN 受控对照**。也就是说，首轮汇报模板保留两条路线的判定口径，但**执行优先级**不是两线并发，而是 **PointCloud 先行、MeshGNN 后置**。
...
**（2026-04-24）补充**：**`V2P-WSSP-01`**（bootstrap、`split_AG_v1`，**仅 p + WSS 监督**）已完成 seed=1，用于 **壁面/WSS 导向** 与 PointNeXt 的联合验证；**不替代**上述 5 组的首轮判定，也不得用其 **`rmse_vel_mag`** 与全速度监督实验比主表。数值与作图口径见 [任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md)「V2P-WSSP-01」。

**（2026-04-25）补充**：**`V2P-WSSP-02`**（**全场 u/v/w/p + WSS 头 + 混合早停**，标准采样）已完成 seed=1，用于与 **WSSP-01** 对照 **场监督对速度与 WSS 的影响**；同样**不替代**首轮 5 组。**WSS 测试 R² 仍未转正**，与 WSSP-01 的 **壁面 WSS 可解释性**不可无注释并列。见 [任务A实验状态表](../03-共享执行与状态/任务A实验状态表.md)「V2P-WSSP-02」。
