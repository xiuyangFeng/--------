...
# ============================================================================
# 使用方法:
#   ./generate_case_list.sh                 # 扫描 AG/fast（默认）
#   ./generate_case_list.sh --slow          # 扫描 AG/slow
#   ./generate_case_list.sh --ag-subdir slow
#   ./generate_case_list.sh CASE1 CASE2     # 指定病例（不扫描）
# ============================================================================
...
if [ ${#POSITIONAL[@]} -gt 0 ]; then
    echo "使用指定的病例列表..."
    > "$CASE_LIST_FILE"
    for case in "${POSITIONAL[@]}"; do
        echo "$case" >> "$CASE_LIST_FILE"
    done
else
    SUB="${SCAN_SUBDIR:-fast}"
    DATA_DIR="$(dirname "$(dirname "$SCRIPT_DIR")")/data_new/AG/$SUB"
