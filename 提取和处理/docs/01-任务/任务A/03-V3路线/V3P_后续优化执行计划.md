...
新的训练必须转到 [V3_发散优化探索路线.md](./V3_发散优化探索路线.md) 的路径 F 门禁：先回填 **TODO-18 MultiK1（5202）**，并完成 **TODO-27a + TODO-53 + TODO-32 + oracle** 的 0 重训诊断，再由证据触发 **TODO-31 / 37 / 42 / 29** 中的一条。若出现混表、条件特征泄漏、或点级升而 Pa/病例级明显退化，该结果不得作为 Go 进入三 seed。
...
- **下一跳 · TODO-18**：多半径 / 多 k 邻域聚合（**单变量**）
- **并行 · TODO-19**：V3D per-domain norm（node03 CPU）
