...
- **（2026-03-31）**：**`A-Opt-03` vs `A-Opt-05` / `A-Opt-05t_*`（P0-5 调参子批，seed=1 子集）** 的 **Fig A3 / A5 / A4** 在 **`outputs/field/plots/optimization/A_Opt05_tune_vs_Opt03_seed1/`**（`plot_taskA_multimodel_scatter` / `plot_taskA_multimodel_regional_bar` / `plot_taskA_multimodel_per_case_boxplot`，`--exp-filter` 限定 `exp_id`）。**读图与正文叙事**以 **`A-Opt-05` 为主视角**（近壁等区域略优），`A-Opt-03` 为轻量对照；后续消融/Line G 母版见状态表「战略锚点」。**壁面 WSS 全量对比图**仍以 **`compare_hemo_wss_runs`** 为准，当前 **暂缓全量导出**；见 [任务A实验状态表](任务A实验状态表.md)「`A-Opt-05_tune`」。
