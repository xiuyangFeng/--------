...
- **当前最大阻塞**：baseline 与主模型的区域标签口径暂不一致；`A-Main-01` 已有完整的高曲率/近壁/分叉区域评估，但 `A-Base-01/02/03` 仍缺完整区域标签，暂时不适合直接横向比较复杂区域
- **下周只做哪 1 到 2 件事**：1. 统一区域标签评估口径并补齐 baseline 的复杂区域结果；2. 启动 A-Abl-01（输入特征层级消融）
...
- 当前最大阻塞已不再是“还没生成预测文件”，而是“baseline 与主模型的复杂区域标签口径不一致”，因此复杂区域横比需要先补齐旧预测资产
