...
| M2  | 完成单尺度核心消融：`A-Abl-01` 到 `A-Abl-05`                 | 2026-04-06 | 进行中  | **`A-Abl-02` ✅（2026-04-09，三 seed）**；**Line G：`G01`/`G04`/`G05` ✅ 三 seed（2026-04-14）**，`G02`/`G03` ⏹ 已结案（仅 seed1，2026-04-17 决定不补种）；**`A-*-wss-multi` ✅ 训练 + 后处理（2026-04-17）**，壁面 WSS 表 `outputs/field/wss_multitask_test_wall_wss_metrics.tsv`，**R² 主表**见 [任务A实验状态表](任务A实验状态表.md)「实验记录摘要 · WSS 多任务」；其余消融子项待启动 |
