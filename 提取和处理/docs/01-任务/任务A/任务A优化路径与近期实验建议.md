...
- **`A-Abl-02` 已完成（2026-04-09）**：**NormRadius** 主贡献、**Tangent** 次之，**Abscissa / Curvature** 边际较弱（详见 [任务A实验状态表](任务A实验状态表.md)）；后续新增特征仍与 **`A-Opt-05`** 一对一对照；
...
| --- | --- | --- | --- | --- | --- | --- |
| `A-Opt-G01` | 距分叉点距离 + 分叉拓扑标记 | 显式编码局部拓扑转折，告诉模型“这里离分叉还有多远” | `bifurcation`、`trunk->branch` 过渡区 | 低 | 低 | **✅ 三 seed（2026-04-14）**；相对 05：`summary.rmse_vel_mag` 略差、`R²_u/p` 略好——见 [任务A实验状态表](任务A实验状态表.md) |
...
**Line G 推荐执行顺序：**（**前置 `A-Abl-02` 已于 2026-04-09 完成**：**NormRadius / Tangent** 主贡献，见 [任务A实验状态表](任务A实验状态表.md)）
...
8. **下一优先**：`A-Abl-01`（输入层级）；**`A-*-wss-multi` 预测与 Fig A3–A5 ✅（2026-04-17，Job 2704/2715）**；**R² 汇报表**见 [任务A实验状态表](任务A实验状态表.md)「实验记录摘要 · WSS 多任务」；或 **Line W**（W01+）
