...
1. `A-Opt-G01`：先补拓扑显式先验；
2. `A-Opt-G02`：再补局部尺度变化信息；
3. `A-Opt-G03 / G04`：视分区域误差走向二选一或顺序推进；
4. `A-Opt-G05`：作为补充项。
...
8. **下一优先**：`A-Abl-01`（输入层级）或 **`A-Opt-G01`**：分叉距离/拓扑标记
...
建议先把 `A-Opt-01 ~ A-Opt-07` 作为近期主线，其余根据结果再决定是否进入正式执行。**（2026-04-02）`A-Opt-07` 已归档**：结论为**负**。**（2026-04-09）`A-Abl-02` 已归档**。后续主线仍以 **`A-Opt-05`** 为母版，优先 **`A-Abl-01` / Line G**。
