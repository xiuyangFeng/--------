...
两条线共享同一个 **P0 组合最优基座**；**（2026-03-28）默认取 `A-Opt-03`**（`target_weights` + Pre-Norm；**`A-Opt-03w` 未更好**），但各自独立归因。
...
- **再下一步（P0-4，✅ 2026-03-28）**：**`A-Opt-03`**（`A-Opt-01` 的 `target_weights` + **`use_transformer_prenorm: true`**）与 **`A-Opt-03w`**（再叠 **`warmup_epochs=5`**）均已 **三 seed 归档**；训练期 **`val`/`best_epoch` 汇总**：`outputs/field/plots/optimization/prenorm_A_Opt03_vs_Opt03w/best_metrics.csv`。  
  - **`A-Opt-03` 结论**：**速度侧主指标相对 `A-Main-01`、`A-Opt-01`、`A-Opt-02` 均为当前 P0 线最优**（`summary.test_metrics.rmse_vel_mag` 三 seed 均值 **~1.031**；`regional_eval` · **`interior.rmse_vel_mag` ~1.822**；**`wall.rmse_vel_mag` 亦优于两条单改线**）；**内部点 `R²_u/v/w` 均值** 高于单独 P0-1 / P0-2。**压力侧**：**`summary.test_metrics.rmse_p` 三 seed 均值**（~**0.642**）**差于 `A-Opt-01` / `A-Opt-02`**（~**0.620 / 0.610**），与低压权一致；**`regional_eval` · `all.rmse_p`** 与 **`A-Opt-02`** 持平，**`interior.rmse_p`** 略差于 **`A-Opt-02`**。  
  - **`A-Opt-03w`**：相对 **`A-Opt-03`** **速度未更好、`rmse_p` 未收复**——**组合线默认不必开 warmup**。  
  - **下一里程碑**：**`A-Opt-04`**（`hidden_dim=256`，在 **`A-Opt-03` 配置**上扩展），见 §「P0-5 推进门槛」。
...
**与本仓库的对齐（2026-03-27，结论延展 2026-03-28）**：P0-3 已落地为 **`A-Opt-02_warmup`**（在 **`A-Opt-02`** 上仅打开上述 warmup），与 [任务A冻结卡](任务A冻结卡.md)、[任务A实验状态表](任务A实验状态表.md) 第三批推荐顺序一致。**`A-Opt-02_warmup`（三 seed）已归档**。**`A-Opt-03` / `A-Opt-03w`（P0-4）已于 2026-03-28 归档**；**当前主执行顺序下一里程碑为 `A-Opt-04`**。训练复现示例（P0-3）：
...
第一批建议只跑下面几组（均 seed=1 先做 smoke test）。**与上文衔接**：**`A-Opt-02`、P0-3 / `A-Opt-02_warmup`（2026-03-27）与 P0-4 / `A-Opt-03`、`A-Opt-03w`（2026-03-28）均已归档**；**下一优先为 `A-Opt-04`（容量）**。
...
- 每组新特征都只和 `A-Main-01` 或 P0 最优配置做一对一对照；
...
>
> **前置条件**：Line W 的所有实验均基于 **P0 组合最优基座 `A-Opt-03`** 起跑（**`A-Opt-03w` 未更好**，除非后续实验特别需要再对比 warmup）。
>
...
推荐执行顺序（均基于 P0 最优基座，seed=1 先做 smoke test）：
...
4. `A-Opt-03w`：叠加 `warmup=5`（✅ 2026-03-28；默认基座仍取 03）
5. `A-Opt-04`：`hidden_dim = 256`（**下一优先**）
6. `A-Opt-05`：`hidden_dim = 256, num_layers = 4`
7. `A-Opt-07`：区域加权 `interior_boost = 3.0`
8. `A-Abl-02`：补齐现有显式几何分量消融
9. `A-Opt-G01`：分叉距离/拓扑标记
10. `A-Opt-G02`：半径变化率/截面积变化率
11. 视区域误差走向尝试 `A-Opt-G03` 或 `A-Opt-G04`
12. 若单尺度收益趋缓，再立项多尺度结构（A-Opt-10）
...
- 每次只新增 1 类特征，其他配置固定为 `A-Main-01` 或 P0 最优版本；
...
> **注意**：Line W 实验与 Line A 实验（A-Opt-01~10）并行推进，各自独立归因。Line W 的基座来自 P0 最优结果，Line A 的基座同样来自 P0 最优结果。
