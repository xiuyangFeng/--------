...
> **（2026-03-26）** 优化线 `A-Opt-01` 在相同口径下已补全 `regional_eval`；默认 **`plot_taskA_multimodel_regional_bar`** 扫描结果包含 **baseline 四组 + `A-Opt-01`**（共 5 组 `exp_id`），需在文中区分「基线四模型对比」与「含 P0-1 的扩展对比」。  
...
| Exp ID    | Model       | Geom | BC  | is_wall | Physics | RMSE_u            | RMSE_v            | RMSE_w            | RMSE_\|v\| (all)  | Interior RMSE_\|v\| | RMSE_p            | R2_p              | Infer(ms)     | Mem(MB)          |
| --------- | ----------- | ---- | --- | ------- | ------- | ----------------- | ----------------- | ----------------- | ----------------- | -------------------- | ----------------- | ----------------- | ------------- | ---------------- |
| A-Base-01 | MLP         | ✗    | ✓   | ✗       | ✗       | 0.9739±0.0009     | 0.9755±0.0012     | 0.9743±0.0008     | 1.5999±0.0015     | **2.6924±0.0242**    | 0.6581±0.0185     | 0.9201±0.0045     | **0.54±0.27** | **127.34±0.00**  |
| A-Base-02 | GraphSAGE   | ✗    | ✓   | ✓       | ✗       | 0.9309±0.0013     | 0.9165±0.0049     | 0.8503±0.0031     | 1.3611±0.0159     | **2.3165±0.0452**    | 0.7341±0.0136     | 0.9007±0.0037     | **2.35±0.23** | **529.69±0.47**  |
| A-Base-03 | Transformer | ✗    | ✓   | ✓       | ✗       | 0.9337±0.0011     | 0.9170±0.0043     | 0.8482±0.0047     | 1.3645±0.0101     | **2.3330±0.0148**    | 0.7061±0.0349     | 0.9079±0.0092     | 6.95±0.09     | 2182.74±1.08     |
| A-Main-01 | Transformer | ✓    | ✓   | ✓       | ✗       | **0.8977±0.0073** | **0.8518±0.0113** | **0.6957±0.0290** | **1.1612±0.0383** | **2.0668±0.0492**    | **0.6536±0.0423** | **0.9209±0.0103** | **6.88±0.02** | **2182.12±0.00** |
| A-Opt-01 | Transformer | ✓    | ✓   | ✓       | ✗       | 0.8737±0.0056 | 0.8231±0.0023 | 0.6502±0.0011 | 1.0811±0.0090 | **1.9187±0.0172**    | 0.6200±0.0362 | 0.9290±0.0082 | — | — |


> **注**：效率图现已包含 `mean±std` 汇总图、分 seed 延迟图、分 seed 显存图、全病例峰值显存图和分 seed Pareto 图；`speedup_vs_CFD` 仍无法填写，因为 `cfd_time_hours` 为空。  
> **`A-Opt-01` 效率列**：与 `A-Main-01` 同结构同实现路径，本次未重跑 `run_efficiency_benchmark`；部署开销可按主模型同款数量级引用，若论文需要独立条请再补测。  
...
- **下一步动作**：按 [优化路径](任务A优化路径与近期实验建议.md) 进入 **P0-2 / `A-Opt-02`（Pre-Norm，须 `use_transformer_prenorm=true` 从头训）** 或先做最小消融记录 P0-1 结论。
