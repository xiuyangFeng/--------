        ...
        try:
            for epoch in range(1, epochs + 1):
                ...
                if self._use_domain_loss and self.norm_stds:
                    ...
                    agent_debug_log(
                        run_id="code-audit",
                        hypothesis_id="H4_val_score_component_scale",
                        location="training/core/trainer.py:FieldTrainer.fit",
                        message="val_score_components",
                        data={
                            "epoch": int(epoch),
                            "wss_term": float(wss_term),
                            "wall_p_rmse": float(wall_p_rmse),
                            "p_term": float(_P_SCALE * wall_p_rmse),
                            "vel_term": float(vel_term),
                            "val_score": float(val_score),
                            "vel_supervised": bool(_vel_supervised),
                            "norm_std_keys": sorted(self.norm_stds.keys()),
                        },
                        max_count=8,
                    )
                    # endregion
