import torch
import pandas as pd
import numpy as np
from torch_geometric.data import Data
from sklearn.neighbors import NearestNeighbors
from sklearn.preprocessing import StandardScaler
import os

# ==========================================
# 1. 图构建模块 (k-NN)
# ==========================================
def build_graph_edges(pos_numpy, k=16):
    """
    构建动态图连接 (k-NN)。
    返回: edge_index [2, E]
    """
    # 使用 sklearn 查找最近邻 (CPU并行加速)
    nbrs = NearestNeighbors(n_neighbors=k + 1, algorithm='auto', n_jobs=-1).fit(pos_numpy)
    _, indices = nbrs.kneighbors(pos_numpy)
    
    # 1. 源节点 (Neighbors): 剔除第一列(自己)，展平
    source_nodes = indices[:, 1:].flatten()
    
    # 2. 目标节点 (Center): 每个点重复 k 次
    num_nodes = pos_numpy.shape[0]
    target_nodes = np.repeat(np.arange(num_nodes), k)
    
    # 3. 堆叠
    edge_index = np.vstack((source_nodes, target_nodes))
    return torch.from_numpy(edge_index).long()

# ==========================================
# 2. 核心转换逻辑
# ==========================================
def csv_to_pyg_data(csv_path, output_path=None, k=20):
    """
    将 CSV 转换为包含 Features(x), Labels(y), Masks 的 PyG Data 对象。
    """
    print(f"🔄 正在读取: {csv_path}")
    df = pd.read_csv(csv_path)
    
    # --- A. 提取坐标 (Pos) ---
    # 原始物理坐标，用于 PINN 物理导数计算 (必须保留真实尺度)
    pos_raw = df[['x', 'y', 'z']].values.astype(np.float32)
    
    # --- B. 构建输入特征 (x) ---
    # 1. 几何特征读取
    abscissa = df['Abscissa'].values.astype(np.float32)
    norm_radius = df['NormRadius'].values.astype(np.float32)
    curvature = df['Curvature'].values.astype(np.float32)
    tangents = df[['Tangent_X', 'Tangent_Y', 'Tangent_Z']].values.astype(np.float32)
    
    # 2. 特征工程
    # (a) 曲率截断与标准化
    curvature = np.clip(curvature, 0.0, 20.0)
    scaler = StandardScaler()
    curvature = scaler.fit_transform(curvature.reshape(-1, 1)).flatten()
    
    # (b) 坐标归一化 (仅用于输入特征 x，不影响 pos)
    centroid = np.mean(pos_raw, axis=0)
    max_dist = np.max(np.linalg.norm(pos_raw - centroid, axis=1))
    pos_normalized = (pos_raw - centroid) / (max_dist + 1e-8)
    
    # 3. 拼接特征矩阵 X [N, 9]
    # [Abscissa(1), NormRadius(1), Curvature(1), Tangents(3), NormPos(3)]
    x_features = np.hstack([
        abscissa.reshape(-1, 1),
        norm_radius.reshape(-1, 1),
        curvature.reshape(-1, 1),
        tangents,
        pos_normalized
    ])
    x_tensor = torch.from_numpy(x_features).float()
    
    # --- C. 构建标签 (y) ---
    # 检查 CSV 中是否包含 CFD 结果 (u, v, w, p)
    y_tensor = None
    has_labels = False
    potential_labels = ['u', 'v', 'w', 'p']
    
    if all(col in df.columns for col in potential_labels):
        print("   ✅ 检测到 CFD 标签 (Training Mode)")
        # 提取 u, v, w, p
        labels = df[potential_labels].values.astype(np.float32)
        y_tensor = torch.from_numpy(labels).float()
        has_labels = True
    else:
        print("   ℹ️ 未检测到 CFD 标签 (Inference Mode)")

    # --- D. 构建边界掩码 (Masks) ---
    # 用于 PINN 区分壁面和内部点
    # 1. 优先使用 'is_wall' 列
    if 'is_wall' in df.columns:
        is_wall = df['is_wall'].values.astype(bool)
    else:
        # 2. 如果没有标签，根据 NormRadius 推断 (大于 0.95 视为壁面)
        # 注意：这是为了场景A (纯几何) 也能运行，但在场景B (训练) 中最好有 is_wall
        print("   ⚠️ 未找到 'is_wall' 列，使用 NormRadius > 0.98 进行推断")
        is_wall = norm_radius > 0.98

    # 转换为 boolean tensor
    mask_wall = torch.from_numpy(is_wall).bool()
    mask_inner = ~mask_wall # 内部点就是非壁面点

    # --- E. 构建图连接 (Edge Index) ---
    edge_index = build_graph_edges(pos_raw, k=k)
    
    # --- F. 封装 Data 对象 ---
    data = Data(
        x=x_tensor,             # 输入: [N, 9]
        pos=torch.from_numpy(pos_raw).float(), # 物理坐标: [N, 3]
        edge_index=edge_index,  # 边: [2, E]
        
        # 辅助掩码 (用于 Loss 计算)
        mask_wall=mask_wall,    # [N] Bool: 哪些是壁面
        mask_inner=mask_inner   # [N] Bool: 哪些是内部流体
    )
    
    # 如果有标签，加入 y
    if has_labels:
        data.y = y_tensor       # 标签: [N, 4] (u,v,w,p)
        
    # 保存元数据 (用于反归一化)
    data.centroid = torch.from_numpy(centroid).float()
    data.scale = float(max_dist)
    
    # --- G. 打印统计与保存 ---
    print("\n📊 数据统计:")
    print(f"   - 节点数: {data.num_nodes}")
    print(f"   - 边数:   {data.num_edges}")
    print(f"   - 内部点: {data.mask_inner.sum().item()} (计算 NS 方程)")
    print(f"   - 壁面点: {data.mask_wall.sum().item()} (计算 No-slip / WSS)")
    if has_labels:
        print(f"   - 标签维度: {data.y.shape} (u, v, w, p)")
    
    if output_path:
        torch.save(data, output_path)
        print(f"\n💾 PyG 数据已保存至: {output_path}")
        
    return data

# ==========================================
# 主程序
# ==========================================
if __name__ == "__main__":
    # 1. 输入文件 (使用刚才生成的 train_data_volumetric.csv)
    # 如果是场景A (只有表面)，就用 train_data_surface.csv
    input_csv = "train_data_volumetric.csv"  
    output_pt = "vessel_graph_training.pt"
    
    # 2. 检查文件是否存在
    if os.path.exists(input_csv):
        # 3. 运行转换
        # k值建议：训练数据较密时 k=20-30，稀疏时 k=10-15
        data = csv_to_pyg_data(input_csv, output_pt, k=20)
        
        # 4. 检查一下 Data 里的内容
        print("\n🔍 Data 对象结构:")
        print(data)
    else:
        print(f"❌ 找不到输入文件: {input_csv}")
        print("请先运行 Script_Scenario_B_Volumetric.py 生成 CSV 文件。")