...
与"体素影像"绑定的环节，因为本项目数据是 Fluent 非结构化点云：
  ...
  1. 网格构建：不再造 UniformGrid / threshold / extract_surface / smooth，
     壁面几何直接取 CFD 壁面节点 + STL 面法向（两者同坐标系）。
  2. 采样方式：不再沿法向取固定 3 层等距点插值，改为取壁面点的 K 近邻
     内部单元，用其到壁面的**法向投影距离** eta 做加权最小二乘拟合。
     理由：CFD 近壁网格是各向异性边界层（法向间距 ~0.18mm，壁面点到最近
     内部点 p50 0.60mm），固定 0.6mm 三层会跨越多层单元并丢失近壁曲率。
  3. 粘度：CFD 用 Carreau-Yasuda **非牛顿**血液模型（见 udf-inlet.c），
     μ 随剪切率在 0.16 → 0.0035 Pa·s 间变化（56 倍跨度）。原脚本固定
     4 cP = 0.0035 Pa·s 只对应高剪切极限，在低剪切区会系统性低估 WSS。
...
MM_TO_M = 1e-3
MU_HIGH_SHEAR = 0.0035  # Pa·s，等于原脚本默认的 4 cP
...
def fit_wall_gradient(
    ...
    参数
    ----
    neighbors : K 近邻个数。K=64 在实测中最优（覆盖边界层又不过度外扩）。
    ...
    for row, (point, normal, idx) in enumerate(zip(wall_mm, normals, nb)):
        # eta = 沿内法向的投影距离（m）；只保留法向正侧（真正在管腔内）的点
        eta = ((interior_mm[idx] - point) @ normal) * MM_TO_M
        keep = eta > 1e-7
        ...
        if bandwidth_weight:
            h = max(float(np.median(e)), 1e-8)
            weight = np.sqrt(np.exp(-((e / (2.0 * h)) ** 2)))
        ...
        try:
            ...
            continue
        gradient[row] = coefficient[0]  # a1 = dv_t/deta |_{eta=0}
        used[row] = len(e)
...
def wss_from_gradient(gradient: np.ndarray, viscosity_model: str) -> np.ndarray:
    ...
    viscosity_model:
      ...
      'newton'  — 固定 0.0035 Pa·s（= 原脚本的 4 cP）
    """
...
def run_case(
    ...
    gradient, used = fit_wall_gradient(
        wall["coords_mm"][idx],
        normals[idx],
        interior["coords_mm"],
        interior["velocity"],
    ...
    pred = wss_from_gradient(gradient, viscosity_model)
