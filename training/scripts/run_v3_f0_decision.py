    ...
    *,
    ...
    def _log(msg: str) -> None:
        if log_fn is not None:
            ...
            print(f"[g0c-pg] {msg}", flush=True)

    split = SplitSpec.from_json(split_path)
    rows: List[Dict[str, object]] = []
    _log(f"pressure-gradient corr start: max_cases={max_cases} max_wall={max_wall_per_graph}")
    for case_rel in split.test_cases[:max_cases]:
        case_dir = REPO_ROOT / "data_new" / "AG" / case_rel / "processed" / "graphs"
        if not case_dir.is_dir():
            continue
        for graph_path in sorted(case_dir.glob("*.pt"))[:max_graphs_per_case]:
            ...
            _log(
                f"  {case_rel}/{graph_path.name}: load {dt_load:.2f}s n_wall={n_wall} "
                f"q={q_idx.size} (step={step}) knn {dt_knn:.2f}s lstsq {dt_lstsq:.2f}s ok={n_mask}"
            )
            if n_mask < 10:
                continue
            rows.append({
