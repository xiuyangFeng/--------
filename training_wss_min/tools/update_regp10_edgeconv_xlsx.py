#!/usr/bin/env python3
"""Update the WSS PointNet workbook with REG-P10 static EdgeConv results."""
...
ANALYSIS = (
    ...
    "regp10_edgeconv_matrix_20260727_results_analysis.json"
)
SECTION_TITLE = "2026-07-28｜REG-P10 静态几何约束 EdgeConv SA1/SA2 矩阵"
ORDER = (
    "edgeconv_sa1_s1234",
    "edgeconv_sa2_s1234",
    "edgeconv_sa12_s1234",
...
LABELS = {
    "edgeconv_sa1_s1234": "REG-P10 + EC-SA1 s1234",
    "edgeconv_sa2_s1234": "REG-P10 + EC-SA2 s1234",
    "edgeconv_sa12_s1234": "REG-P10 + EC-SA12 s1234",
}
CHANGES = {
    "edgeconv_sa1_s1234": "static EdgeConv stages=[1]",
    "edgeconv_sa2_s1234": "static EdgeConv stages=[2]",
    "edgeconv_sa12_s1234": "static EdgeConv stages=[1,2]",
}
DECISIONS = {
    "go_to_three_seed_confirmation": "晋级动态图/多种子确认",
...
def overview_values(record: dict) -> list:
    ...
    values[0:4] = [
        ...
        "PointNeXt-R + LocalGeoPE + static EdgeConv",
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="wss_regp10ec_xlsx_") as temp_dir:
        ...
        if len(title_rows) > 1:
            raise RuntimeError("duplicate REG-P10 EdgeConv summary sections")
        if title_rows:
        ...
        if not set(ORDER).issubset(saved_ids):
            raise RuntimeError("overview readback missing REG-P10 EdgeConv rows")
        saved_labels = {
            values[0]
        ...
        if not set(LABELS.values()).issubset(saved_labels):
            raise RuntimeError("teacher readback missing REG-P10 EdgeConv rows")
        saved_titles = {
            values[0]
        ...
        if SECTION_TITLE not in saved_titles:
            raise RuntimeError("summary readback missing REG-P10 EdgeConv section")
        check.close()
