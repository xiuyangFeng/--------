#!/usr/bin/env python3
"""Submit the approved five-task single-seed D2-K64 wave-1 matrix."""
...
STATIC_AUDIT = REPO / "training_wss_min/preflight/d2_k64_ilo_structure_wave1_static_audit.json"
OUTPUT = REPO / "training_wss_min/preflight/d2_k64_ilo_structure_wave1_submission.json"
GATE_SCRIPT = REPO / "training_wss_min/cluster/preflight_d2_k64_ilo_structure_wave1.slurm"
TRAIN_SCRIPT = REPO / "training_wss_min/cluster/run_d2_k64_ilo_structure_wave1.slurm"
...
def main() -> None:
    ...
    if manifest.get("status") != "prepared" or len(rows) != 5:
        raise RuntimeError("wave1 manifest is not the approved exact five-task matrix")
    ...
    if {row["seed"] for row in rows} != {1234}:
        raise RuntimeError("wave1 contains a non-1234 seed")
    for row in rows:
    ...
    payload = {
        ...
        "policy": "five new seed1234 jobs; one common gate; array 0-4 concurrent",
        "jobs": [],
    }
    try:
        if args.dry_run:
            ...
            gate = submit(
                "--job-name=d2k64ilo_pf", f"--export={export}", str(GATE_SCRIPT)
            )
            payload["jobs"].append({"role": "formal_gate", "job_id": gate})
            train = submit(
                "--job-name=d2k64ilo", f"--export={export}",
                f"--dependency=afterok:{gate}", "--array=0-4%5", str(TRAIN_SCRIPT),
            )
            payload["jobs"].append({
