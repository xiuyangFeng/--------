#!/usr/bin/env python3
"""Submit the S3-GEOPE three-seed regularization matrix."""
...
OUTPUT = PREFLIGHT / "s3_regularization_matrix_submission.json"
GATE = ROOT / "training_wss_min/cluster/preflight_s3_regularization_matrix.slurm"
RUN = ROOT / "training_wss_min/cluster/run_s3_regularization_matrix.slurm"
MAX_CONCURRENT = 4
...
def main() -> None:
    ...
    parser.add_argument("--existing-gate")
    ...
    array_spec = f"0-{len(rows) - 1}%{MAX_CONCURRENT}"
    payload = {
        "schema_version": 1,
        ...
        "policy": (
            "S3-GEOPE anchor; four strict single-variable regularizers; "
            "three paired seeds; one formal gate; afterok array"
        ),
        "tasks": len(rows),
        "array": array_spec,
        "jobs": [],
    ...
    try:
        if args.dry_run:
            ...
            gate = args.existing_gate or sbatch(
                "--job-name=s3reg_pf", f"--export={export}", str(GATE)
            )
            array = sbatch(
                "--job-name=s3reg",
                f"--export={export}",
            ...
            payload["jobs"] = [
                {
                    "role": "formal_gate",
                    "job_id": gate,
                    "reused": bool(args.existing_gate),
                },
                {
